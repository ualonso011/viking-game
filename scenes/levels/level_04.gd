...
func _ready() -> void:
	...
	if hud and hud.has_method("show_level_name"):
		hud.show_level_name("Warpath", 3.0)

	if player:
		game_state.last_checkpoint = player.global_position
		game_state.checkpoint_level = "res://scenes/levels/level_04.tscn"
...
func _on_end_trigger_entered(body: Node) -> void:
	if body.is_in_group("player"):
		...
		# Upgrade max HP at this milestone
		game_state.upgrade_max_hp(1)
		var main = get_node_or_null("/root/Main")
		if main and main.has_method("load_level"):
			game_manager.load_level("res://scenes/levels/level_05.tscn")
